...
GAME_QUESTION_SERVICE = 'game-countdown'
GAME_ANSWER_SERVICE = 'game-countdown'
GAME_LEADERBOARD_SERVICE = 'game-countdown'
...
# GAME
    ...
    def test02_gameCountdown(self):
        ...
        game = Game.objects.filter(publicId=session['publicId']).first()
        self.assertEqual(game.state, WAITING)

        # first the game countdown appears:
        ...
        # finally, the leaderboard appears at the end of the game
        self.client1.get(reverse(GAME_QUESTION_SERVICE), follow=True)
        session = self.client1.session
        game = Game.objects.filter(publicId=session['publicId']).first()
